...
import { addGameToCollection, addUserGame, extractIgdbImageIdFromUrl, listUserCollections, upsertGameFromIGDB } from '@/services/profile';
            ...
            <View style={{ gap: 10, marginTop: 8, alignSelf:'stretch' }}>
              <Pressable onPress={() => handleChoose('playing')} style={[styles.optionBtn]} disabled={!canAdd || !!busy}>
                {busy === 'playing' ? (
                  ...
                  <Text style={styles.optionTxt}>Jogando Atualmente</Text>
                )}
              </Pressable>
              <Pressable onPress={() => handleChoose('wishlist')} style={[styles.optionBtn]} disabled={!canAdd || !!busy}>
                {busy === 'wishlist' ? (
                  ...
                  <Text style={styles.optionTxt}>Quero Jogar</Text>
                )}
              </Pressable>
              {collections.map((c) => (
