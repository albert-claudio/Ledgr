...
import { usePopularThisWeek, useSearchGames, useSearchSuggestions, useWeeklySteamReviews } from '../../hooks/useSearch';
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 80 }} keyboardShouldPersistTaps="handled">
        ...
        {showSugg && q.length >= 2 && (sugg.data || []).length > 0 && (
          <View style={styles.suggBox}>
            {(sugg.data || []).slice(0, 5).map((g, idx) => (
              <Pressable key={`${g.id}-${idx}`} onPress={() => { setText(g.name); setShowSugg(false); }} style={styles.suggItem}>
          ...
          <>
            ...
            {reviews.isLoading ? (
              ...
              (reviews.data || []).map((r, idx) => (
                <View key={`${r?.review?.recommendationid ?? 'rev'}-${idx}`} style={styles.reviewCard}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                    <View style={styles.avatar} />
                    <Text style={styles.reviewHeader}>Jogador Steam  <Text style={{ fontWeight: 'bold' }}>{r.gameName}</Text></Text>
                  </View>
