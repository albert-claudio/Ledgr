...
import { getPlatformsForIgdbIds, getUserCollectionSamples, igdbCoverUrl, listUserCollections, UserCollection } from '@/services/profile';
...
export default function CollectionScreen() {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={{ paddingTop: 16, paddingBottom: 42 }} stickyHeaderIndices={[0]}>
        {/* Search Bar */}
        <View style={{ paddingHorizontal: 16, paddingBottom: 12, backgroundColor: colors.background }}>
          <View style={styles.searchBar}>
            <Ionicons name="search" size={16} color={colors.secondary} />
            <Text style={styles.searchPlaceholder}>Pesquise sua coleção...</Text>
          </View>
        </View>
        ...
        <Pressable onPress={() => setShowCreate(true)} style={styles.backlogBtn}>
          <Text style={styles.backlogTxt}>Atualize sua Coleção</Text>
        </Pressable>

        <ShelfRow title="Jogando" count={playing.data?.length || 0} covers={playingCovers} platformIds={playingIds} onPress={() => router.push('/collection/status/playing')} />
        <View style={styles.separator} />

        <ShelfRow title="Quero Jogar" count={wishlist.data?.length || 0} covers={wishlistCovers} platformIds={wishlistIds} onPress={() => router.push('/collection/status/wishlist')} />
        <View style={styles.separator} />
  ...
  return (
    <Pressable style={styles.rowWrap} onPress={onPress as any}>
      ...
      <View style={styles.metaBlock}>
        ...
        <Text style={styles.shelfCount}>{count} jogos</Text>
...
const styles = StyleSheet.create({
  ...
  searchBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1A', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10 },
  searchPlaceholder: { ...typography.caption, color: colors.secondary, marginLeft: 8 },
});
