...
import { useRouter, Link } from 'expo-router'; import { useLocalSearchParams } from 'expo-router';
...
export default function LoginScreen() {
  ...
  async function handleLogin() {
    ...
    // sucesso: o listener do _layout pode redirecionar; se não tiver listener, força aqui:
              ...
              <View style={styles.form}>
                ...
                <View style={{ marginTop: 14 }}>
                  <TouchableOpacity
                    ...
                    onPress={async () => {
                      try {
                        await signInWithOAuth('google', ['email', 'profile']);
                        setOk('Login realizado!'); setSuccessVisible(true); try { require('expo-haptics').notificationAsync(require('expo-haptics').NotificationFeedbackType.Success); } catch {} setTimeout(() => router.replace('/(tabs)'), 2000);
                      } catch (e: any) {
                        setErr(e?.message || 'Falha no login social.');
                      }
                    }}
                  >
