...
import { SafeAreaView, View, Text, TouchableOpacity, StyleSheet, ActivityIndicator, Linking, Alert } from 'react-native';
...
export default function CheckEmail() {
  ...
  async function resend() {
    ...
    setLoading(false);
    if (error) {
      Alert.alert('Erro', 'Não consegui reenviar agora. Tenta de novo em 1 min.');
      return;
    }
    setSent(true);
  }
  ...
  function openMailApp() {
    // tenta abrir o Gmail; se não rolar, cai num mailto:
    Linking.openURL('mailto:').catch(() => {
      Alert.alert('Dica', 'Abra seu app de e-mail e busque pelo assunto de confirmação do Ledgr.');
    });
  }
    ...
    <SafeAreaView style={s.wrap}>
      <View style={s.card}>
        ...
        <TouchableOpacity style={[s.ghost, { marginTop: 10 }]} onPress={() => router.replace('/(auth)/login')}>
          <Text style={s.ghostTxt}>Já confirmei — Voltar ao login</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={s.linkBtn} onPress={resend} disabled={loading}>
          {loading ? <ActivityIndicator color="#9edfe0" /> : (
            <Text style={s.linkTxt}>{sent ? 'E-mail reenviado! (confira o spam)' : 'Reenviar e-mail de confirmação'}</Text>
          )}
        </TouchableOpacity>
      </View>
    </SafeAreaView>
